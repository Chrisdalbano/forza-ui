...
Copy this directory into `.smoke/nuxt` at the repository root. Run `npm run build:lib` in the root and `npm pack --workspace packages/vue --pack-destination .smoke`. In the consumer directory, run `npm install` and `npm install ../chrisdalbano-forza-ui-0.3.2.tgz`, then `npm run build` and `node .output/server/index.mjs`.
